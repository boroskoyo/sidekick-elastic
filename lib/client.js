...
var sidekickConnect = function (clientInfo) {
  const sidekick_host = clientInfo.sidekick_host ? clientInfo.sidekick_host:'wss://broker.service.runsidekick.com';
  const sidekick_port = clientInfo.sidekick_port ? clientInfo.sidekick_port:443;
  const token = clientInfo.sidekick_token;
  const email = clientInfo.sidekick_email;
  const password = clientInfo.sidekick_password;
  const tracepointFunction = clientInfo.tracepointFunction ? clientInfo.tracepointFunction : (data) => {console.log({"type":"log event",data})};
  const logpointFunction = clientInfo.logpointFunction ? clientInfo.logpointFunction : (data) => {console.log({"type":"log event",data})};
